...
import io.github.garyttierney.ghidralite.core.search.index.IndexChange
...
data class ProgramChange<T>(val eventType: Int, val program: Program, override val value: T) : IndexChange<T>
...
class ProgramChangeWatcher(private val scope: CoroutineScope) : DomainObjectListener {
    ...
    fun <T : Any> registerInterest(group: ProgramChangeInterest<T>): Flow<ProgramChange<T>> {
        group.events.forEach { interestBits.set(it) }
    ...
    override fun domainObjectChanged(ev: DomainObjectChangedEvent) {
        ...
        scope.launch {
            ...
            for (change in ev) {
                ...
                val info = interestTable[change.eventType]!!
